    ...
    public void work() {
        ...
        for(int i=0;i<listPackages.length && listPackages[i]!=null;i++) {
            if (listPackages[i].getStatus() == Status.CREATION1) {
                int j = 0;
                while (j<listTrucks.length && listTrucks[j].available == false) {//search non busy truck
                    j++;
                }
                ...
                listPackages[i].setStatus(Status.BRANCH_TRANSPORT6);//TODO use BRANCH_TRANSPORT6
